...
@interface GPUImageMovieWriter ()
{
    ...
    CMTime startTime, audioStartTime, previousFrameTime, previousAudioTime;
...
- (id)initWithMovieURL:(NSURL *)newMovieURL size:(CGSize)newSize fileType:(AVFileType)newFileType outputSettings:(NSMutableDictionary *)outputSettings;
{
    if (!(self = [super init]))
    {
        return nil;
    ...
    _shouldStartWritingVideoFrameFirst = NO;
    _stopWritingAudioFrameImmediatelyWhenFinished = NO;
...
- (void)startRecording;
{
    canWriteAudio = !_shouldStartWritingVideoFrameFirst;
    alreadyFinishedRecording = NO;
    startTime = kCMTimeInvalid;
    audioStartTime = kCMTimeInvalid;
    runSynchronouslyOnContextQueue(_movieWriterContext, ^{
...
//    if (_hasAudioTrack && CMTIME_IS_VALID(startTime))
    if (_hasAudioTrack)
    {
        ...
        {
            if (_shouldStartWritingVideoFrameFirst)
                return;
